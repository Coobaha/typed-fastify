...
expectType<Service<ExampleSchema>>({
  "GET /": (req, reply) => {
    ...
    const reply1 = knownReply200.send(user);
    return reply1;
  },
  "POST /": (req, reply) => {
    //@ts-expect-error
...
type ComplexHandlers = RequestHandler<
  ExtendedSchema,
  "GET /" | "POST /" | "PATCH /" | "PUT /other" | "PATCH /other"
>;
//<editor-fold desc="shared wtf handler request/reply types">
const dd = (
  ...
  function testHeaders(
    headers: typeof req["headers"] | typeof reply["request"]["headers"]
  ) {
  ...
  if (reply.matches("GET /")) {
    ...
    return replyWithSecondHeader
      .header("postHeaderRes2", "332")
      .header("postHeaderRes", "3")
      .send(user);
...
const testHandler = async (
  req: TestHandlers["Request"],
  reply: TestHandlers["Reply"]
): TestHandlers["ReturnAsync"] => {
